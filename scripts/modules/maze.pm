...
sub build {
    ...
    if (-d $binary_dir and -e $binary) {
        ...
        system ("cp $binary $binary_dir/maze.backup$new_version");
    } elsif (! -d $binary_dir) {
    ...
    $build_command .= " $src_dir/maze.c -o $binary_dir/maze";
...
sub fuzz {
    ...
    my $binary = "$workspace/binaries/$binary_context/maze";
    if (! -e $binary) {
    ...
    if ($resume) {
        ...
        my $seeds_directory = "$RESOURCES/seeds/maze";
        $fuzz_command .= " -i $seeds_directory";
    }

    $fuzz_command .= " -o $results_dir -T \"maze-$experiment_name-$exec_context\"";
