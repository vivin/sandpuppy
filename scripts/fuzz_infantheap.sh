...
if [[ $# -lt 3 ]]; then
    echo "Syntax: $0 <experiment-name> <waypoints> <path/to/seeds> [save_all] [with_asan]"
    exit 1
fi
...
echo -n "Checking if infantheap binary exists..."

infantheap_binary_dir=$BINARIES/$experiment_name/infantheap
if [ ! -f "$infantheap_binary_dir/infantheap" ]; then
    echo "no"

    $SCRIPTS/build_infantheap.sh $experiment_name $waypoints $with_asan
    if [ $? -ne 0]; then
...
experiment_results_directory=$RESULTS/$experiment_name/infantheap
...
if [ "$waypoints" != "none" ]; then
    $FUZZ_FACTORY/afl-fuzz $SAVE_ALL_FLAG -p -i $seeds_directory -o $fuzz_output_directory -T "infantheap-$experiment_name" -R $trace_directory $MEM_LIMIT_OPTION $infantheap_binary_dir/infantheap
else
    $FUZZ_FACTORY/afl-fuzz $SAVE_ALL_FLAG -i $seeds_directory -o $fuzz_output_directory -T "infantheap-$experiment_name" $MEM_LIMIT_OPTION $infantheap_binary_dir/infantheap
fi
