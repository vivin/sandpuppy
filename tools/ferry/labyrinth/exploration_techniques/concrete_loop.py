import logging
import angr
from angr.analyses.loopfinder import Loop
from angr.knowledge_base import KnowledgeBase
from angr.knowledge_plugins.functions import Function

l = logging.getLogger("labyrinth.exploration_techniques.concrete_loop")


class ConcreteLoop(angr.ExplorationTechnique):
    """
    This exploration technique handles symbolic loops.
    For loop whose index or limit is symbolic, we try to reduce the state fork
    by assign concrete values to it.
    """

    CONCRETIZATION_NUM = 2

    def __init__(self, cfg=None, concretizaiton_num=CONCRETIZATION_NUM, functions=None, loops=None):
        """
        :param cfg          : Normalized CFG is required.
        :param functions    : Function(s) containing the loop(s) to be analyzed.
        :param loops        : Loop(s) to be analyzed.
        """

        super(ConcreteLoop, self).__init__()
        self.cfg = cfg
        self.concretizaiton_num = concretizaiton_num
        self.functions = functions

        self.loops = {}

        if type(loops) is Loop:
            loops = [loops]

        if type(loops) in (list, tuple) and all(type(l) is Loop for l in loops):
            for loop in loops:
                self.loops[loop.entry_edges[0][0].addr] = loop
        elif loops is not None:
            raise TypeError('What type of loop is it?')

    def setup(self, simgr):
        if self.cfg is None:
            cfg_kb = KnowledgeBase(
                self.project, self.project.loader.main_object)
            self.cfg = self.project.analyses.CFGFast(kb=cfg_kb, normalize=True)
        elif not self.cfg.normalized:
            l.warning("LoopSeer uses normalized CFG. Recomputing the CFG...")
            self.cfg.normalize()

        if type(self.functions) is str:
            func = [self.cfg.kb.functions.function(name=self.functions)]
        elif type(self.functions) is int:
            func = [self.cfg.kb.functions.function(addr=self.functions)]
        elif type(self.functions) is Function:
            func = [self.functions]
        elif type(self.functions) in (list, tuple):
            func = []
            for f in self.functions:
                if type(f) is str:
                    func.append(self.cfg.kb.functions.function(name=f))
                elif type(f) is int:
                    func.append(self.cfg.kb.functions.function(addr=f))
                elif type(f) is Function:
                    func.append(f)
                else:
                    raise TypeError("What type of function is it?")
        elif self.functions is None:
            func = None
        else:
            raise TypeError("What type of function is it?")

        if not self.loops or func is not None:
            loop_finder = self.project.analyses.LoopFinder(
                kb=self.cfg.kb, normalize=True, functions=func)

            for loop in loop_finder.loops:
                entry = loop.entry_edges[0][0]
                self.loops[entry.addr] = loop

    def step(self, simgr, stash, **kwargs):
        kwargs['successor_func'] = self.normalized_step

        simgr.step(stash=stash, **kwargs)

        for state in simgr.stashes[stash]:
            # Processing a currently running loop
            if state.loop_data.current_loop:
                loop = state.loop_data.current_loop[-1][0]
                header = loop.entry.addr

                if state.addr == header:
                    state.loop_data.trip_counts[state.addr][-1] += 1
                elif state.addr in state.loop_data.current_loop[-1][1]:
                    # This is for unoptimized while/for loops.
                    #
                    # 0x10812: movs r3, #0          -> this block dominates the loop
                    # 0x10814: str  r3, [r7, #20]
                    # 0x10816: b    0x10868
                    # 0x10818: movs r3, #0          -> the real loop body starts here
                    # ...
                    # 0x10868: ldr  r3, [r7, #20]   -> the loop header is executed the first time without executing the loop body
                    # 0x1086a: cmp  r3, #3
                    # 0x1086c: ble  0x10818
                    back_edge_src = loop.continue_edges[0][0].addr
                    back_edge_dst = loop.continue_edges[0][1].addr
                    block = self.project.factory.block(back_edge_src)
                    if back_edge_src != back_edge_dst and back_edge_dst in block.instruction_addrs:
                        state.loop_data.trip_counts[header][-1] -= 1

                    state.loop_data.current_loop.pop()

                l.debug("%s trip counts %s", state,
                        state.loop_data.trip_counts)

            # Loop entry detected. This test is put here because in case of
            # nested loops, we want to handle the outer loop before proceeding
            # the inner loop.
            if state.addr in self.loops:
                loop = self.loops[state.addr]
                header = loop.entry.addr
                exits = [e[1].addr for e in loop.break_edges]

                state.loop_data.trip_counts[header].append(0)
                state.loop_data.current_loop.append((loop, exits))

        return simgr

    def normalized_step(self, state):
        node = self.cfg.get_any_node(state.addr)
        if state.loop_data.current_loop:
            loop = state.loop_data.current_loop[-1][0]
            if loop.break_edges:
                break_edge_srcs = [e[0].addr for e in loop.break_edges]
                break_edge_dsts = [e[1].addr for e in loop.break_edges]
                # for state at the exit of a symbolic loop, we assign different
                # values to limiter and go on
                loop_succs = []
                break_succs = []
                if state.addr in break_edge_srcs:
                    successors = state.step(num_inst=len(
                        node.instruction_addrs) if node is not None else None)
                    if len(successors.flat_successors) > 1:
                        l.info("[Bug]: Symbolic loop in block 0x%x", state.addr)

                        for succ in successors.flat_successors:
                            if succ.addr in break_edge_dsts:
                                # for state to break the loop, we do not concretize
                                # anything
                                break_succs.append(succ)
                            else:
                                loop_succs.append(succ)

                        for loop_succ in loop_succs:
                            successors.all_successors.remove(loop_succ)
                            successors.successors.remove(loop_succ)
                            successors.flat_successors.remove(loop_succ)

                            guard = loop_succ.scratch.guard
                            symbolic_args = [
                                arg for arg in guard.args if arg.symbolic]
                            if len(symbolic_args) != 1:
                                l.info("Loop guard contains %d symbolic argument(s), skip it", len(
                                    symbolic_args))
                            else:
                                arg = symbolic_args[0]
                                candidates = loop_succ.solver.eval_upto(
                                    arg, self.concretizaiton_num)
                                max_candidate = loop_succ.solver.max(arg)
                                min_candidate = loop_succ.solver.min(arg)
                                if max_candidate not in candidates:
                                    candidates.append(max_candidate)
                                if min_candidate not in candidates:
                                    candidates.append(min_candidate)
                                l.info(
                                    "Concretization candidates: %s", candidates)
                                for candidate in candidates:
                                    tmp_succ = loop_succ.copy()
                                    tmp_succ.solver.add(arg == candidate)
                                    successors._categorize_successor(tmp_succ)

                        return successors

        return state.step(num_inst=len(node.instruction_addrs) if node is not None else None)
