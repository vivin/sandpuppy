...
class SimSolver(SimStatePlugin):
    ...
    def independent_constraints(self):
        return self._solver.independent_constraints()
    ...
    @ast_stripping_decorator
    @error_converter
    def same_constraints(self, c1, c2):
        return not self.satisfiable(extra_constraints=(c1 != c2,))

    @timed_function
    @ast_stripping_decorator
    @error_converter
    def same_expressions(self, e1, e2):
        try:
            c_e1 = self.eval_one(e1)
            c_e2 = self.eval_one(e2)
            return c_e1 == c_e2
        except (SimValueError, SimSolverModeError):
            pass

        if self.min(e1) != self.min(e2):
            return False
        return self.max(e1) == self.max(e2)

    @timed_function
    @ast_stripping_decorator
    @error_converter
    def same_range(self, e1, e2, extra_constraints=()):
        return self._solver.same_range(e1, e2, extra_constraints=extra_constraints)

    @timed_function
    @ast_stripping_decorator
    @error_converter
    def will_change(self, c):
        return self.satisfiable(extra_constraints=(self.Not(c),))

    @timed_function
    @ast_stripping_decorator
    @error_converter
    def replace_all_variables(self, e, extra_constraints=()):
        return self._solver.replace_all_variables(e, extra_constraints=extra_constraints)
    ...
    def extract_variables(self, e):
        if not self.symbolic(e):
            return set()

        return self._solver._extract_variables(e)
