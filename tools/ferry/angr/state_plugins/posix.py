...
class SimSystemPosix(SimStatePlugin):
    ...
    def fstat(self, fd): #pylint:disable=unused-argument
        ...
        if self.state.solver.symbolic(fd):
            ...
            size = self.state.solver.BVS('st_size', 64, key=('api', 'fstat', 'st_size'))
        else:
            ...
            size = self.state.posix.get_fd(fd).size() if fd > 2 else self.state.se.BVS('st_size', 64, key=('api', 'fstat', 'st_size'))
