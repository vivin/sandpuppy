    ...
    def read(self, pos, size, **kwargs):
        ...
        if self.state.solver.symbolic(size):
            try:
                ...
                passed_max_size = self.state.solver.eval_one(size)
...
def _generate_file_content_in_bytes(state, start_idx, size):
    symbolic_data = None
    for offset in range(0, size):
        symbolic_byte = state.solver.BVS(
            "filewrap_symbolic_%s" % (start_idx + offset),
            8,
            key=("filewrap", "symbolic", start_idx + offset),
            eternal=True
        )

        if symbolic_data is None:
            symbolic_data = symbolic_byte
        else:
            symbolic_data = symbolic_data.concat(symbolic_byte)
...
class SimFileDescriptor(SimFileDescriptorBase):
    ...
    def read_data(self, size, **kwargs):
        symbolic_wrap = kwargs.pop("symbolic_wrap", False)
        switch_offset = kwargs.pop("switch_offset", -1)
        in_bytes = kwargs.pop("in_bytes", True)

        size = self._prep_read(size)
        data, realsize, self._pos = self.file.read(self._pos, size)

        if symbolic_wrap:
            c_pos = self.state.solver.eval_one(self._pos)
            c_realsize = self.state.solver.eval_one(realsize)

            if c_realsize == 0:
                return data, realsize

            data = data.get_bytes(0, c_realsize)

            start_idx = c_pos - c_realsize
            end_idx = c_pos
            if switch_offset < 0 or end_idx <= switch_offset:  # wrap all contents with symbolic value
                wrapped_data = self.state.solver.BVS(
                    "filewrap_concrete_%s_%s" % (start_idx, c_realsize),
                    c_realsize * 8,
                    key=("filewrap", "concrete", start_idx, c_realsize),
                    eternal=True
                )
                if data.concrete:
                    self.state.add_constraints(wrapped_data == data)
                return wrapped_data, realsize
            elif start_idx >= switch_offset:  # return pure symbolic value with `filewrap` key
                if in_bytes:
                    symbolic_data = _generate_file_content_in_bytes(self.state, start_idx, c_realsize)
                else:
                    symbolic_data = self.state.solver.BVS(
                        "filewrap_symbolic_%s_%s" % (start_idx, c_realsize),
                        c_realsize * 8,
                        key=("filewrap", "symbolic", start_idx, c_realsize),
                        eternal=True
                    )
                return symbolic_data, realsize
            else:  # start < switch_offset < end, return mixed value
                c_data_len = switch_offset - start_idx
                wrapped_data = self.state.solver.BVS(
                    "filewrap_concrete_%s_%s" % (start_idx, c_data_len),
                    c_data_len * 8,
                    key=("filewrap", "concrete", start_idx, c_data_len),
                    eternal=True
                )
                if data.concrete:
                    self.state.add_constraints(wrapped_data == data.get_bytes(0, switch_offset - start_idx))

                s_data_len = end_idx - switch_offset
                if in_bytes:
                    symbolic_data = _generate_file_content_in_bytes(self.state, switch_offset, s_data_len)
                else:
                    symbolic_data = self.state.solver.BVS(
                        "filewrap_symbolic_%s_%s" % (switch_offset, s_data_len),
                        s_data_len * 8,
                        key=("filewrap", "symbolic", switch_offset, s_data_len),
                        eternal=True
                    )
                return wrapped_data.concat(symbolic_data), realsize
...
class SimFileDescriptorDuplex(SimFileDescriptorBase):
    ...
    def read_data(self, size, **kwargs):
        if "symbolic_wrap" in kwargs or "switch_offset" in kwargs:
            l.warning("Unsupported symbolic wrap for SimFileDescriptorDuplex")
