...
class fstat(angr.SimProcedure):
    ...
    def run(self, fd, stat_buf):
        if self.state.fs.concrete_fs:
            ...
            filepath = simfd.file.name.replace("file://", "")
            stat = self.state.fs.stat(filepath)
    ...
    def _store_real_amd64(self, stat_buf, stat):
        def store(offset, val): return self.state.memory.store(stat_buf + offset, val, endness='Iend_LE')
