...
class fgets(angr.SimProcedure):
    ...
    def run(self, dst, size, file_ptr, symbolic_wrap=False, switch_offset=-1):
        ...
        # the newline and we don't have any notion of buffering in-memory
            ...
            count = 0
            while count < size - 1:
                data, real_size = simfd.read_data(1, symbolic_wrap=symbolic_wrap, switch_offset=switch_offset)
        ...
        # caveat: there could also be no newline and the file could EOF.
            ...
            data, real_size = simfd.read_data(size-1, symbolic_wrap=symbolic_wrap, switch_offset=switch_offset)
