...
from angr.sim_type import SimTypeInt, SimTypeLength, SimTypeTop
...
class memset(angr.SimProcedure):
    ...
    def run(self, dst_addr, char, num):
        ...
        if self.state.solver.symbolic(num):
            ...
            try:
                max_size = self.state.solver.eval_one(num)
            except (SimUnsatError, SimValueError):
                max_size = self.state.solver.max_int(num)
                l.warn("symbolic memset(), use max possible value: %d", max_size)
