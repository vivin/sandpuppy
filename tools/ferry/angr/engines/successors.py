                ...
                if len(cond_and_targets) > max_targets:
                    ...
                    for cond, a in cond_and_targets:
                        ...
                        if o.KEEP_IP_SYMBOLIC in split_state.options:
                            ...
                            if split_state.has_plugin("runtime_state"):
                                rs_summary = RuntimeStateSummary(split_state)
                                rs_transition = RuntimeStateTransition(
                                    split_state, RuntimeStateTransition.RST_INDIRECT_JUMP, split_state.scratch.exit_ins_addr,
                                    indirect_jump_cond=cond,
                                    indirect_jump_target=a,
                                )
                                updated = rs_transition.update_rs_summary(rs_summary)
                                if updated:
                                    if state.project.kb.runtime_states.reached(rs_summary, state):
                                        l.info("reached runtime state in jump table 0x%x",
                                               split_state.scratch.exit_ins_addr)
                                    pass
...
from ..state_plugins.runtime_state_utils import RuntimeStateSummary, RuntimeStateTransition
